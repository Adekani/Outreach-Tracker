...
const DEFAULT_DATA = () => ({
  ...
  streak: {
    ...
    // v2: freeze tokens replace old freeze system
    freezeTokens: 0,
    freezeUsedThisWeek: false,
    // kept for backward compat read, never written
...
function recalcStreak() {
  ...
  if (lastActive === todayStr || lastActive === yesterdayStr) {
    // Streak is alive — count consecutive days back
    ...
    // useFreezeToken() handles advancing lastActiveDate before this runs when the user taps "Use Freeze",
    // so if we reach here the streak genuinely ended and no active freeze covers it.
    state.streak.current = 0;
    state.streak.lastActiveDate = lastActive;
    // Reset weekly freeze tracking for the new streak
...
function useFreezeToken() {
  ...
  // Can only use when at-risk or next day (streak just broken by 1 day)
  ...
  // Deduct token and cover the gap
  state.streak.freezeTokens = Math.max(0, state.streak.freezeTokens - 1);
  // Mark yesterday as covered by injecting a synthetic flag
  // We extend the lastActiveDate to "today" so streak recalc considers it alive
  // The actual streak count remains the same, but streak is no longer at risk
  state.streak.lastActiveDate = today();
  state.streak.freezeUsedThisWeek = true;
  saveData(state);
  renderAll();
  showToast('❄️ Freeze token used! Streak protected for today.', 'success');
}
